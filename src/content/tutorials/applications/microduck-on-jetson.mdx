...
Everything here was verified on three machines with the CLI at version **0.9.4**. The per-box tabs below say what was verified where, and when.
...
| **Jetson AGX Thor** | JetPack 7, L4T R38.2.2, CUDA 13.0, Python 3.12 | Simulation, CLI, training smoke (with the torch-source fix described in its tab). Verified 2026-09-04 headless; operator re-run in progress. |
| **Jetson AGX Orin** | L4T R39, CUDA 13.2, Python 3.12 | Simulation and CLI only. Verified 2026-09-04 headless; operator re-run in progress. |
...
- **git**, and about 30 GB of free disk for the two clones, the Rust build and the RL virtual environment.
...
Every verification record behind this tutorial ran at CLI version 0.9.4, which is why the install line pins it. `microduck` and `microduck-cli` are the same entry point. Agents start with `microduck learn`, which prints the command map, the exit-code contract and the `--json` output contract; `microduck explain <noun> <verb>` describes any single verb.
...
The `UV_INDEX` override keeps `uv` from downloading a multi-gigabyte `torch` build that cannot run here; `onnxruntime`, `mujoco` and the rest resolve normally. Any later bare `uv run` inside the clone re-locks toward upstream's index, so prefix `uv` calls there with the same variable. The CLI never runs `uv` inside the clone: `env up` invokes the virtual environment's Python directly.
...
The doctor exits `2` and names every failing check when the box is not ready. Do not proceed on a `2`. The one expected exception is Thor on the pull-request branch, where `rl_pinned_commit` fails by design (see the Thor tab).
...
`--sim` starts the real `duck-body` MuJoCo simulator behind `robotd --sim` and waits up to 120 seconds for the duck to report healthy. A viewer window appears on your display. Two variants:

```bash
microduck env up --sim --headless --skip-build   # same body, no window
microduck env up --fake                          # robotd --fake: no physics, no window
```

Drop `--skip-build` the first time if you skipped Step 3; `env up` runs the cargo build itself.
...
Run one of the daemon's one-shot skills. Never guess a skill name; ask the live daemon what it has:
...
The engine is the only process that opens the duck's control socket. It logs its start sequence on stderr (connect, hello, health, init, enable, armed), then every rule firing and every drop with a named reason:
...
A rule that fires and a human typing an intent are judged by the same admission registry. With no engine live, this validates the intent and prints what would be admitted, sending nothing:
...
On the GB10 the smoke completed in about a minute (`real 0m58.6s` in the [Spark record](https://github.com/agentculture/microduck-cli/blob/main/docs/verification/2026-09-04-sim-bringup.md)), with `ok: true` in the JSON payload.
...
Expected while the `microduck_rl` clone is on the pull-request branch from the Thor tab. Every other check should pass. It clears when [#39](https://github.com/pollen-robotics/microduck_rl/pull/39) merges and the CLI re-pins.
...
The body cannot open a window without a display. Over SSH, export the desktop session's `DISPLAY` and `XAUTHORITY` (see "Watching the window from a headless session"), or add `--headless` and use `duck monitor --json` instead.
