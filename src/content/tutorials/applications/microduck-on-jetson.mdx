...
The Spark path was followed end to end on **2026-09-07** with the CLI at version **0.9.4** from PyPI. On **2026-09-08** the operator followed it on Jetson AGX Thor with the same wheel: the training smoke passed there, and the simulation's memory footprint was measured on Thor and on Jetson AGX Orin ([record](https://github.com/agentculture/microduck-cli/blob/main/docs/verification/2026-09-08-thor-orin-tutorial-run.md)). The 2026-09-04 Thor and Orin records, which ran earlier checkouts of the CLI (`2b00480` and `3c09fb0`, version 0.9.1), remain the reference for the simulation and control steps on those boards. The per-box tabs below say what was verified where, and when.

## Why Jetson?

MicroDuck ships with its own onboard computer, which runs `robotd` and the policies on the real robot. A Jetson does not compete with it: it can replace that board as the duck's central unit, or sit beside it as the machine that runs the simulation, the training and the agent driving the CLI, with the same daemon and the same wire protocol on both. The duck's control loop, its ONNX policies and the MuJoCo physics all run on one machine. Jetson AGX Thor and DGX Spark carry the unified memory and the Blackwell-class GPU that `mujoco-warp` and `torch` want for training thousands of parallel environments, so the same board that will eventually sit inside a robot can also train the policy it runs. Jetson AGX Orin runs the simulation, the daemon and the CLI comfortably; at the current upstream pins its `sm_87` GPU has no matching `torch` wheel for training, so this tutorial treats Orin as a simulation and control target only.
...
|---|---|---|
| **Jetson AGX Thor** | JetPack 7, L4T R38.2.2, CUDA 13.0, Python 3.12 | Simulation, CLI, training smoke (with the torch-source fix described in its tab). Verified 2026-09-04 headless; the smoke re-run on the 0.9.4 wheel on 2026-09-08 beside a running production stack. |
| **Jetson AGX Orin** | L4T R39, CUDA 13.2, Python 3.12 | Simulation and CLI only. Verified 2026-09-04 headless; simulation memory footprint measured 2026-09-08. |
...
<Warning title="On a Jetson, check out the torch-source fix instead of the pinned microduck_rl commit">
At the pinned `microduck_rl` commit, `torch` is routed to a CUDA 12.9 index on every `linux-aarch64` host, which has no kernels for Thor's `sm_110` GPU. The fix is open as [pollen-robotics/microduck_rl#39](https://github.com/pollen-robotics/microduck_rl/pull/39) (issue [#38](https://github.com/pollen-robotics/microduck_rl/issues/38)); until it merges, a Jetson checks out the pull request's head commit. Do this now, not in Step 4: on the 2026-09-08 Thor run the clone was moved back to the pinned commit after the fix had been applied, and the next `uv run` quietly re-installed the CUDA 12.9 `torch`, which failed the smoke with "no kernel image is available for execution on the device" ([record](https://github.com/agentculture/microduck-cli/blob/main/docs/verification/2026-09-08-thor-orin-tutorial-run.md)). `env doctor` reports the mismatch as a warning; that is expected.
</Warning>

On **Jetson AGX Thor and Jetson AGX Orin**:
...
On **DGX Spark**, the pinned commit as shipped:

```bash nocheck
git -C ~/git/microduck_rl checkout 29e887ecfbf5d37144759e5a9f8a176dfb83d547
```
...
<Tabs labels={["Jetson AGX Thor", "Jetson AGX Orin", "DGX Spark"]}>
<div class="nv-tab-panel active">

<Info title="Verified 2026-09-04 (headless); smoke re-run 2026-09-08 on the 0.9.4 wheel">
The [2026-09-08 record](https://github.com/agentculture/microduck-cli/blob/main/docs/verification/2026-09-08-thor-orin-tutorial-run.md) covers the operator's run on the PyPI wheel beside Thor's production containers: the smoke passed in 26 seconds, and the simulation's footprint was measured. The [2026-09-04 record](https://github.com/agentculture/microduck-cli/blob/main/docs/verification/2026-09-04-thor-sanity.md) has the full sanity pass and the torch resolution.
</Info>

With the clone at `a30a9e4` from Step 2:
...
`torch` 2.9.1 arrives as a native `linux_aarch64` wheel from `pypi.jetson-ai-lab.io/sbsa/cu130`, with the three wheels it links but does not declare (`nvpl-blas`, `nvpl-lapack`, `nvidia-cudss-cu13`). The download is several gigabytes; give it time, and keep the `UV_HTTP_TIMEOUT` so a slow mirror does not abort the sync. No `LD_LIBRARY_PATH` is needed: the branch pre-loads those libraries when the `mjlab_microduck` package is imported, which every train and play path does before `torch`.

<Note title="Probing torch by hand on this branch">
A bare `python -c "import torch"` in the virtual environment fails with `ImportError: libnvpl_lapack_lp64_gomp.so.0` because nothing pre-loaded the library. Import the package first:

```bash nocheck
cd ~/git/microduck_rl
.venv/bin/python -c "import mjlab_microduck.tasks, torch; print(torch.__version__, torch.cuda.get_arch_list())"
```

On Thor this prints `2.9.1 ['sm_110', 'sm_121']`.
</Note>

</div>
<div class="nv-tab-panel">

<Info title="Verified 2026-09-04 (headless); footprint measured 2026-09-08">
...
You still need the virtual environment for `onnxruntime`, which `robotd` loads for the policies. With the clone at `a30a9e4` from Step 2 the same `uv sync` as on Thor resolves `torch` from NVIDIA's SBSA index (the branch keys that index on the `-tegra` kernel, which Orin has too):
...
If you kept the clone at the pinned `develop` commit instead, override the index by hand, and prefix every later `uv` call in the clone the same way, or a bare `uv run` re-locks toward upstream's index and starts a multi-gigabyte re-download:

```bash nocheck
UV_INDEX="pytorch-cu129=https://pypi.jetson-ai-lab.io/sbsa/cu130" uv sync
```

Either way `torch` installs and initialises CUDA but cannot train on `sm_87`; `onnxruntime`, `mujoco` and the rest resolve normally. `env up` does not go through `uv` (it runs the virtual environment's Python directly), so the simulation is unaffected.
...
<Note title="This step takes a while">
The virtual environment weighs about 7.5 GB once resolved (`torch`, the CUDA libraries it pulls, `warp-lang`, `mujoco-warp`), and `uv` downloads all of it on the first sync. Expect minutes on a fast link and considerably longer on a slow one; `UV_HTTP_TIMEOUT=600` keeps a slow mirror from aborting the run. Later syncs are quick.
</Note>
...
The doctor exits `2` and names every failing check when the box is not ready. Do not proceed on a `2`. A pin mismatch is only a warning, so a Jetson with `microduck_rl` on the pull-request branch from Step 2 still gets a `healthy` verdict, with one `[WARN] rl_pinned_commit` line naming both commits; it is the verdict line that matters.

<Note title="On the 0.9.4 wheel: two [FAIL] lines under a healthy verdict">
Version 0.9.4 from PyPI prints `[FAIL] microduck_pinned_commit: pinned commit for microduck is unknown (docs/upstream-pins.md unreadable or the row is missing)` and the same for `rl_pinned_commit`, while the first line still says `healthy` and the exit code is `0`: the wheel did not carry the pins file the check read, and a warning-severity check was printed with the `[FAIL]` prefix. Both are fixed in `microduck-cli` 0.9.6 ([agentculture/microduck-cli#10](https://github.com/agentculture/microduck-cli/pull/10)): the pins ship inside the package, so a wheel verifies both clones, and warnings print `[WARN]`. On 0.9.4, compare the commits yourself with `git -C ~/git/microduck rev-parse HEAD` against the table in Step 2 and read the verdict line.
</Note>
...
<Info title="What the simulation costs in memory">
Measured on 2026-09-08 from `/proc/<pid>/status` with the duck standing ([record](https://github.com/agentculture/microduck-cli/blob/main/docs/verification/2026-09-08-thor-orin-tutorial-run.md)): the MuJoCo body (`python -m mjlab_microduck.sim.body_server`) held 933 MB of resident memory on Jetson AGX Thor with the viewer window open and 590 MB on Jetson AGX Orin headless; `robotd --sim` held 34 MB and 41 MB. Box-wide, Orin's `used` rose by 0.9 GB while the stack was up, beside its production containers with 8 GB available. The simulation is about a gigabyte; only the training smoke in Step 10 needs real headroom.
</Info>
...
On 2026-09-08, on the 0.9.4 wheel with 36 GB available beside Thor's production stack (three vLLM engines and their services), the smoke reported `ok: true` after 26 seconds on a warm kernel cache with a peak of 2.8 GB resident ([record](https://github.com/agentculture/microduck-cli/blob/main/docs/verification/2026-09-08-thor-orin-tutorial-run.md)). The [2026-09-04 record](https://github.com/agentculture/microduck-cli/blob/main/docs/verification/2026-09-04-thor-sanity.md) shows the cold-cache figure, 144 seconds, where one kernel alone took 25 seconds to compile. If the box also runs other services, confirm they are still healthy afterwards; on 2026-09-08 every container was still up and the engines `healthy`:

```bash nocheck
docker ps
```

<Warning title="If it fails with 'no kernel image is available for execution on the device'">
The virtual environment holds the CUDA 12.9 `torch` again. That is what happened on the first 2026-09-08 attempt: the clone had been moved back to the pinned commit after Step 2, and `policy smoke`'s `uv run` re-synced `torch` to match (`Uninstalled 1 package … Installed 1 package` at the top of its stderr). Check out `a30a9e4` and `uv sync` again, then re-run.
</Warning>
...
There is no `torch` wheel with `sm_87` kernels at these pins, so `policy smoke` fails on AGX Orin with "no kernel image is available". Everything up to Step 9 works, and costs about 0.6 GB of memory (Step 6); run the smoke on Thor, Spark or Hugging Face Jobs.
</Warning>
...
<details class="nv-details">
<summary>env doctor on a Jetson: rl_pinned_commit is a mismatch</summary>
<div class="nv-details-content">

Expected while the `microduck_rl` clone is on the pull-request branch from Step 2: `[WARN]` on 0.9.6, `[FAIL]` on the 0.9.4 wheel (which prints `microduck_pinned_commit … unknown` as well, Step 5's note). Both are warnings and the verdict stays `healthy`. It clears when [#39](https://github.com/pollen-robotics/microduck_rl/pull/39) merges and the CLI re-pins. Do not "fix" it by checking the pinned commit back out on a Jetson; that re-installs the `torch` that cannot run on Thor (Step 10's Thor tab).

</div>
</details>

<details class="nv-details">
<summary>import torch fails with ImportError: libnvpl_lapack_lp64_gomp.so.0</summary>
<div class="nv-details-content">

You imported `torch` by hand in the `microduck_rl` virtual environment on the pull-request branch. The branch pre-loads that library when `mjlab_microduck.tasks` is imported, which every `train` and `play` path does first; a bare probe does not. Use the probe in the Thor tab of Step 4, or run the smoke, which is unaffected.
...
- [microduck-cli](https://github.com/agentculture/microduck-cli) and its verification records for [DGX Spark](https://github.com/agentculture/microduck-cli/blob/main/docs/verification/2026-09-04-sim-bringup.md) (and the [2026-09-07 re-test that followed this tutorial](https://github.com/agentculture/microduck-cli/blob/main/docs/verification/2026-09-07-spark-retest.md)), [Jetson AGX Thor](https://github.com/agentculture/microduck-cli/blob/main/docs/verification/2026-09-04-thor-sanity.md), [Jetson AGX Orin](https://github.com/agentculture/microduck-cli/blob/main/docs/verification/2026-09-04-orin-sanity.md) and the [2026-09-08 Thor and Orin run on the wheel](https://github.com/agentculture/microduck-cli/blob/main/docs/verification/2026-09-08-thor-orin-tutorial-run.md)
...
- [microduck-cli#10](https://github.com/agentculture/microduck-cli/pull/10): `env doctor` pins packaged into the wheel, `[WARN]` for warnings (0.9.6)
