...
<div class="admonition note">
  ...
  <p>Prebuilt Yocto images are published from the <a href="https://developer.nvidia.com/embedded/jetpack/downloads" target="_blank" rel="noopener">JetPack SDK downloads page</a> (see the <strong>Yocto Images</strong> section / the <a href="https://developer.nvidia.com/embedded/jetpack/downloads/archive-7.2" target="_blank" rel="noopener">JetPack 7.2 downloads archive</a>). If you'd rather build the image yourself, jump to <a href="#from-quick-start-to-production">From quick start to production</a>.</p>
</div>
...
| Jetson AGX Thor T4000 | `jetson-agx-thor-t4000` | `demo-image-full-jetson-agx-thor-t4000.rootfs.tegraflash-tar.zst` | NVMe (`nvme0n1p1`) |
...
<div class="admonition warning">
  ...
  <p>For the three NVMe targets, <strong>install the NVMe drive in the Jetson before</strong> putting the device into recovery mode. The flash script writes the root filesystem to the storage target described by the <code>MACHINE</code> configuration.</p>
</div>
...
From the [JetPack downloads page](https://developer.nvidia.com/embedded/jetpack/downloads) (JetPack 7.2 / Jetson Linux R39.2), download the `.tegraflash-tar.zst` package whose `MACHINE` matches your hardware (see the [Supported images](#supported-images) table). Save it to `~/Downloads` and **leave the filename unchanged**.
...
<Tabs labels={["Thor AGX devkit", "Thor AGX T4000", "Orin AGX 64 GB", "Orin Nano (NVMe)"]}>
  ...
  <div class="nv-tab-panel">
    <pre style="background: #1e293b; color: #e2e8f0; padding: 1rem; border-radius: 8px;"><code>{"export MACHINE=\"jetson-agx-thor-t4000\""}</code></pre>
  </div>
  <div class="nv-tab-panel">
...
|---------------|-------------------|---------|
| Jetson AGX Thor devkit / T4000 | USB-C port with Force-Recovery functionality (**`5a`**, next to the HDMI connector); use the other USB-C port (`5b`) for power | [Hardware Layout](https://docs.nvidia.com/jetson/agx-thor-devkit/user-guide/latest/hardware_layout.html) |
...
<Tabs labels={["Thor / Orin AGX (buttons)", "Orin Nano (header pins)"]}>
  <div class="nv-tab-panel active">
    <p>For kits with <strong>RECOVERY</strong> and <strong>RESET</strong> buttons (Thor AGX devkit, Thor AGX T4000, Orin AGX devkit):</p>
    <ol>
...
| Target | Next step |
|--------|-----------|
| Thor AGX devkit / Thor AGX T4000 | Power cycle or reset the Jetson. |
...
|----------|---------------|
| Thor AGX devkit / Thor AGX T4000 | USB CDC-ACM serial from the USB-C port hidden under the lid above the rear ports |
...
- [JetPack SDK downloads](https://developer.nvidia.com/embedded/jetpack/downloads): JetPack 7.2 / Jetson Linux R39.2, including the Yocto images ([7.2 archive](https://developer.nvidia.com/embedded/jetpack/downloads/archive-7.2))
