...
<div class="admonition warning">
  ...
  <p>Many BE6500-class USB adapters ship with an internal flash partition that enumerates as a USB CD-ROM drive (<code>0bda:1a2b</code>) containing a Windows <code>Setup.exe</code>. Linux loads <code>usb-storage</code>, leaving the wireless radio dark. Two distinct actions are required: a <strong>storage quirk</strong> (<code>usb_storage.conf</code>) that stops the kernel binding a storage driver to the installer partition, and the <code>usb_modeswitch</code> command — the step that actually switches the adapter to its network product ID. The quirk suppresses the storage claim; the switch turns the radio on. Single-state adapters (no installer partition) need neither.</p>
</div>
...
flowchart TD
    ...
    B --> C{"Check VID:PID"}
    C -- "0bda:1a2b" --> D["Virtual CD-ROM trap<br/>usb_storage.conf quirk (suppression)<br/>then usb_modeswitch (mode switch)"]
...
### Dual-mode USB bus anchoring (SuperSpeed switch)

Realtek RTL8922AU/8912AU silicon (and BE-class Realtek USB adapters generally) is **dual-mode**: the same dongle enumerates as full-speed on the USB 2.0 fallback path (Bus 001 at 480M) or SuperSpeed on Bus 002 at 5000M depending on which host port it occupies. This is not user-visible at plug time, but it caps the host transfer rate: a 480M anchor soft-limits real throughput at about 400 Mbit/s regardless of the radio's negotiated PHY rate.

The morrownr `rtw89` driver performs the mode switch **for you at module load** — `rtw89_usb_switch_mode_be()` runs as part of probe when the module parameter `switch_usb_mode` is Y (default), programs the pad controller for a SuperSpeed transition, and the device re-enumerates onto the SuperSpeed bus — but only if it is plugged into a port *routed* to Bus 002. Verify the anchoring, not the descriptor:

```bash
lsusb -t
# Look for the adapter's line:
# Port 002: Dev 008, If 0, Class=Vendor Specific Class, Driver=rtw89_8922au_git, 5000M
```

- `480M` (Bus 001): anchored to the USB 2.0 fallback path. Re-plug into a SuperSpeed-routed port and reload the module.
- `5000M` (Bus 002): mode switch engaged, USB bus no longer the bottleneck.

Do not conclude the adapter is "USB 2.0-only" from its descriptor (`bcdUSB 0x0200` is what a dual-mode adapter reports while running as high-speed): the empirical `lsusb -t` check after replug is the definitive verification.
...
| :--- | :--- | :--- | :--- | :--- |
| **USB** | `0bda:1a2b` | Realtek Multi-State | `usb-storage` (trap) | Add `usb_storage.conf` quirk (suppress storage binding), then run `usb_modeswitch -v 0bda -p 1a2b` to switch to network ID |
...
| **PCIe** | `8086:2725` | Intel AX210 | `iwlwifi` | In-kernel driver; decompress BOTH `/lib/firmware/iwlwifi-ty-a0-gf-a0.pnvm` and the matching `iwlwifi-ty-a0-gf-a0-*.ucode` from their `.zst` copies |
...
`CONFIG_MODULE_SIG_ALL=y` is set, but NVIDIA does not distribute `signing_key.pem` in public headers. Plain `make` produces unsigned `.ko` binaries. Check what the running kernel actually enforces before deciding what to do:

```bash
zcat /proc/config.gz | grep CONFIG_MODULE_SIG
```

- `# CONFIG_MODULE_SIG_FORCE is not set` (the JetPack 7.2 default): an unsigned module loads and the kernel records taint bits `4096` (out-of-tree) and `8192` (unsigned) in `/proc/sys/kernel/tainted`. Taint records a load that already happened; it never waives a check that is about to happen.
- `CONFIG_MODULE_SIG_FORCE=y`: unsigned modules are rejected at load time. Sign them with a trusted key:

  ```bash
  /lib/modules/$(uname -r)/build/scripts/sign-file sha512 <signing-key.pem> <signing-cert.der> <module>.ko
  ```

  The `sign-file` helper ships in the headers tree (resolves via the `build` symlink); the key and certificate do not. This kernel sets `# CONFIG_SECONDARY_TRUSTED_KEYRING is not set`, so there is no runtime path to enroll your own certificate: the module must be signed with the kernel build key, or the kernel rebuilt with yours.
...
Unplug and reconnect the adapter. The quirk suppresses the storage claim; the adapter still needs the mode switch itself:

```bash
sudo usb_modeswitch -v 0bda -p 1a2b
```

`lsusb` should now display `0bda:8912`. Confirm the re-enumeration in `dmesg` (look for the disconnect/reconnect pair).
...
For DKMS-managed persistence across kernel updates, the README's documented invocation registers the cloned tree directly:

```bash
sudo dkms install "$PWD"
```

If DKMS stops at the signing step (see the signing section above), fall back to the manual `make install` path and prove the unsigned build loads before re-attempting DKMS.

### Step 5: Decompress firmware and load module

If `/lib/firmware/rtw89/` holds no `rtw8922a_fw-*.bin.zst` at all, install the firmware set first from the cloned `rtw89` tree:

```bash
sudo make install_fw
```

Then decompress the primary firmware and fallback versions into raw `.bin` format:
...
<div class="admonition warning">
  <p class="admonition-title">⚠️ Band steering: locate the 5 GHz BSSID explicitly</p>
  <p>On merged dual-band SSIDs (Xiaomi: <code>Wi-Fi 多频合一</code>), NetworkManager frequently lands the adapter on the 2.4 GHz radio of the merged SSID — capped around 300 Mbit/s (HT MCS15 40 MHz) no matter what the 5 GHz band can do. Locate the per-band BSSIDs explicitly and pin the intended band:</p>
  <p><code>nmcli -f SSID,BSSID,CHAN,SIGNAL dev wifi list ifname &lt;usb-if&gt;</code> — the SSID appears once per band, each with its own BSSID. Then connect with the BSSID for the 5 GHz radio: <code>sudo nmcli dev wifi connect "YourNetworkSSID" ifname &lt;usb-if&gt; bssid &lt;5GHz-BSSID&gt; password "YourPassword"</code>. If the 5 GHz entry is missing from the list, <code>sudo nmcli dev wifi rescan ifname &lt;usb-if&gt;</code> first — one missed scan is a cache race, not absence of coverage.</p>
</div>
<div class="admonition warning">
  <p class="admonition-title">⚠️ Brand-new Wi-Fi 7 routers: association may be silently rejected</p>
  <p>If the connect step keeps failing (<code>ssid-not-found</code>) while other devices (phones, laptops) join the same SSID fine, the AP is likely rejecting association at the feature-negotiation phase. Wi-Fi 7 routers commonly default to MLO (Multi-Link Operation) / TWT / band-merge that the out-of-tree <code>rtw89</code> USB branch cannot negotiate — the kernel log shows a clean <code>send auth → authenticated</code> followed by an un-answered association request, repeated every ~35 s. Turn on the router's <strong>Wi-Fi 5 / legacy compatibility mode</strong> (on Xiaomi BE3600 CN firmware: <code>Wi-Fi 5 兼容模式</code>, which also hard-disables MLO and TWT) and the same <code>nmcli</code> command succeeds immediately. This drops the link to VHT (Wi-Fi 5) coding — on a verified setup, RX still negotiated 1733 Mbit/s PHY (VHT-MCS9, 160 MHz, NSS 2).</p>
  <p>Diagnose the failure mode first: <code>sudo dmesg -w | grep -E 'auth|assoc'</code> — <code>authenticated</code> without a following <code>associated</code> is the feature-negotiation signature; a <code>4-way handshake failed</code> or <code>reason=2</code> deauth is instead a credentials problem.</p>
</div>
...
Instead, pin client traffic per interface using socket-level device binding. `iperf3 -B <IP>` alone binds only the socket's *source address* — pass `--bind-dev <interface>` as well for the real `SO_BINDTODEVICE` semantics (requires iperf 3.16+ and `CAP_NET_RAW`, so prefix with `sudo` where needed):
...
# Confirm the binding landed on the intended interface, not merely the right source address:
ip -s link show wlP1p1s0
```
...
Both interfaces deliver equivalent throughput (~185 to 212 Mbit/s) because of the **two-hop hairpin penalty**: in a client-to-client test through one AP, every frame is transmitted twice over the same half-duplex channel (A → AP → B), halving usable airtime before TCP overhead is counted. The negotiated PHY rate describes *one* crossing by *one* radio — not the end-to-end achievable rate.

Why a 1733 Mbit/s PHY still tops out near 250 Mbit/s end-to-end: (1) the hairpin divides airtime by two, and (2) the client-side PHY rate assumes ideal modulation at that instant, while the actual throughput also carries retransmissions and contention overhead. To exercise the adapter's own headroom, place the iperf3 peer on a **wired host** — that test exercises a single crossing per side and unlocks the 500-700+ Mbit/s single-hop class.

To bench the SuperSpeed-anchored state with the AP at 160 MHz, re-run the two-hop pair after moving the dongle to a Bus 002 port:

| Bus anchoring | Negotiated PHY | TCP TX | TCP RX | Dominant bottleneck |
| :--- | :--- | :--- | :--- | :--- |
| USB 2.0 (Bus 001, 480M) @ 160 MHz | ~400 Mbit/s host cap | 70–92 Mbit/s | 145–170 Mbit/s* | USB 2.0 bus ceiling |
| SuperSpeed (Bus 002, 5000M) @ 160 MHz | 1733.3 Mbit/s TX / 1560.0 Mbit/s RX | 236–262 Mbit/s | 145–170 Mbit/s | two-hop shared airtime |

*The USB-2.0 row's TCP RX number was not recorded for that configuration; the SuperSpeed row is the verified comparison. Either way the 160 MHz/SuperSpeed combination lifts TX throughput ~2.8× over the USB-2.0 TX baseline, and the remaining ceiling is airtime, not the bus.
...
| :--- | :--- | :--- |
| `lsusb` shows `0bda:1a2b` (CD-ROM) | Multi-state USB hardware mode trap | Add `options usb-storage quirks=0bda:1a2b:i` to `/etc/modprobe.d/usb_storage.conf` (suppress storage binding), replug, then run `sudo usb_modeswitch -v 0bda -p 1a2b` (the actual mode switch). |
...
| `Key was rejected by service` | Signature enforcement rejected the module (CONFIG_MODULE_SIG_FORCE set, or signed with an untrusted key) | Read enforcement first: `zcat /proc/config.gz \| grep CONFIG_MODULE_SIG`. With `CONFIG_MODULE_SIG_FORCE=y` no taint state waives the check — sign the `.ko` with a trusted key (`/lib/modules/$(uname -r)/build/scripts/sign-file sha512 ...`) and load again. |
...
| OPEN auth succeeds but association never completes (`send auth` → `authenticated` → repeated `aborting authentication ... by local choice`, no `AssocResp`) while other client types join the same AP | Wi-Fi 7 AP feature negotiation (MLO/TWT/band-merge) rejected by the out-of-tree rtw89 USB branch — verified on Xiaomi BE3600 (fw 1.0.87) with band-merge + MLO defaults; `dmesg -w` shows the loop every ~35 s, NetworkManager reports `ssid-not-found` | Router-side fix (verified): enable the router's **Wi-Fi 5 compatibility mode** (小米路由器: `Wi-Fi 5 兼容模式`) — it forcibly disables MLO/TWT and association succeeds immediately. When available, disable MLO/TWT individually first; on Xiaomi CN firmware they are hard-locked off by compat mode. Not a password, MAC-filter, or driver-flags problem — the AP silently drops the assoc request. |
...
- [host/wifi-bench.sh](https://github.com/explicitcontextualunderstanding/jetson-uefi-rescue-toolkit/blob/main/host/wifi-bench.sh): Dual-interface benchmark automation script using SO_BINDTODEVICE socket binding (pass `--bind-dev <iface>` alongside `-B` for iperf 3.16+).
